...
while IFS= read -r -d '' bin; do
  ...
  if [[ "$bin_name" == "harmonic_series" ]]; then
    ...
    for method in "${!methods[@]}"; do
      name=${methods[$method]}
      sleep 5
      if /usr/bin/time -v "${bin}" "$method" 4e9  > "${name}.log" 2>&1; then
  ...
  sleep 5
  if /usr/bin/time -v "${bin}" > "${bin_name}.log" 2>&1 ; then
    parse_log "${bin_name}.log" "${bin_name}"
  else
    echo "ERROR: ${bin_name} returned non-zero exit status"
  fi
done < <(find ../../bin -maxdepth 1 -type f -executable -print0)
